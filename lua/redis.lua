...
function network.write(client, buffer)
    local _ = client.network.socket:send(buffer)
end

function network.read(client, len)
    if len == nil then len = '*l' end
    local line = client.network.socket:receive(len)
    return line
end
...
function response.read(client)
    querying = true
    local bailid = dfhack.timeout(1000, "frames", function() if querying == true then bail = true end end)
    local payload = client.network.read(client)
    while payload == nil do
        payload = client.network.read(client)
        if bail == true then return end
    end
    dfhack.timeout_active(bailid, nil)
...
local function create_connection(parameters)
    ...
    perform_connection, socket = connect_tcp, require('plugins.luasocket').tcp
